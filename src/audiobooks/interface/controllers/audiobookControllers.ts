...
export class AudiobookControllers {
  ...
  async createAudioBook(req: Request, res: Response): Promise<Response> {
    try {
      const { title, author, descriptions, subgenre, available, language, yearBook, summary, genreType }: PropAudiobooks = req.body;
      ...
      const content = { idAudio: audiobookSaved.public_id, url_secura: audiobookSaved.secure_url };
      const coverImage = { url_secura: coverImageSaved.secure_url, idCoverImage: coverImageSaved.public_id };
  ...
  async deleteAudiobook(req: Request, res: Response): Promise<Response> {
    try{
      ...
      if (!Types.ObjectId.isValid(id)) return res.status(404).json({ msg: "para eliminar un audiolibro debes ingresar una id valida" });
      ...
      if (!deletingStatus) return res.status(500).json({ msg: "no sea podido eliminar el Audiolibro de la base de datos" });
  ...
  async getAudioBookById(req:Request,res:Response): Promise<Response>{
    try{
      ...
      if (!Types.ObjectId.isValid(id)) return res.status(404).json({ msg: "para obtener un audiolibro debes ingresar una id valida" });
