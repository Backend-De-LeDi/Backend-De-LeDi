...
export class DocumentsDrivers implements DocumentsRepository {
  async insertBookToDocuments(bookId: Types.ObjectId): Promise<void> {
    ...
    const authorsData = authorDocs.map((a) => ({ author_mongo_id: a._id.toString(), author_full_name: a.fullName }));
  ...
  async insertAuthorToDocuments(authorId: Types.ObjectId): Promise<void> {
    ...
    const libros = books.map((b) => ({ id_mongo: b._id.toString(), title: b.title }));
    ...
    const { error } = await supabaseAuthors.from("documents").insert({ content, metadata, embedding });

    console.log("✅ autor almacenado correctamente en la base de datos vectorial");

    if (error) console.error("Error insertando autor:", error.message);
  }

  async deleteBookFromDocuments(bookId: string): Promise<void> {
    const { error } = await supabaseAuthors.from("documents").delete().eq("metadata->>id_mongo", bookId.toString());

    if (error) console.error("Error eliminando documento:", error);
  }

  async deleteAuthorFromDocuments(idAuthor: string): Promise<void> {
    const { error } = await supabaseAuthors.from("documents").delete().eq("metadata->>id_mongo", idAuthor.toString());
