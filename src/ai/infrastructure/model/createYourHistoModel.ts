...
export const createYourHistoModel = z.object({
  ...
  ecenary: z.string().min(1, "El escenario debe tener contenido narrativo").describe("Texto narrativo que representa el contenido del escenario"),
...
export const quizModel = z.object({
  ...
  score: z.number().min(0, "La puntuación mínima es 0").max(1000, "La puntuación máxima es 1000").describe("Puntuación que obtendrá el usuario si responde correctamente a la pregunta"),
});

export const finalQuiz = z.object({
  textCompleted: z.string().min(1, " texto de felicitaciones ").describe("texto de felicitaciones por completar el quiz "),
  title: z.string().min(1, "titulo del juego ").describe("el titulo debe ser simpre el original "),
  completed: z.boolean().describe("se debe maracar en true para completarlo"),
});
