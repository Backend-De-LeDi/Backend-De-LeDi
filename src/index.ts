...
import { MongoCrudMetric } from "./metrics/infrastructure/mongo/mongoCrudMetric";
import { GetBookMetric } from "./metrics/app/crud/getMetric";


const getBookMetric = new GetBookMetric(new MongoCrudMetric())
...
io.on("connection", async (socket: Socket) => {
	...
	socket.on("getMetric", () => {
		try {
			setTimeout(async () => {
				const result = await getBookMetric.exec(); // o await getBookMetric() si ya es una promesa
				io.emit("bookMetric", result); // 🔁 emite a todos los clientes
				console.log("[METRICAS] Emitidas:", result);
